...
func TestSavePost_Success(t *testing.T) {
	...
	post.PersonName = "julia"
	post.NumberOfPeople = "3"
	post.RemindDate = "2023-07-12"
	post.RemindTime = "07:12"

	repo := PostRepositoryInit(conn)

	f, saveErr := repo.Save(&post)
	assert.Nil(t, saveErr)
	assert.EqualValues(t, f.PersonName, "julia")
	assert.EqualValues(t, f.NumberOfPeople, "3")
	assert.EqualValues(t, f.RemindDate, "2023-07-12")
	assert.EqualValues(t, f.RemindDate, "07:12")
